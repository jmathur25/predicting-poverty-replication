...
# tza13.cons <- read.dta('data/input/LSMS/TZA_2012_NPS-R3_v01_M_STATA8_English_labels/ConsumptionNPS3.dta') %$%
#   data.frame(hhid = y3_hhid, cons = expmR/(365*adulteq))
# tza13.cons$cons <- tza13.cons$cons*112.69/(585.52*mean(c(130.72,141.01)))
# tza13.geo <- read.dta13('data/input/LSMS/TZA_2012_NPS-R3_v01_M_STATA8_English_labels/HouseholdGeovars_Y3.dta')
# tza13.coords <- data.frame(hhid = tza13.geo$y3_hhid, lat = tza13.geo$lat_dd_mod, lon = tza13.geo$lon_dd_mod)
# tza13.hha <- read.dta('data/input/LSMS/TZA_2012_NPS-R3_v01_M_STATA8_English_labels/HH_SEC_A.dta')
# tza13.rururb <- data.frame(hhid = tza13.hha$y3_hhid, rururb = tza13.hha$y3_rural, stringsAsFactors = F)
# tza13.weight <- read.dta('data/input/LSMS/TZA_2012_NPS-R3_v01_M_STATA8_English_labels/HH_SEC_A.dta')[,c('y3_hhid', 'y3_weight')]
# names(tza13.weight) <- c('hhid', 'weight')
# tza13.hhi <- read.dta('data/input/LSMS/TZA_2012_NPS-R3_v01_M_STATA8_English_labels/HH_SEC_I.dta')
...
## Malawi ##
mwi13.cons <- read.dta('data/input/LSMS/MWI_2016_IHS-IV_v02_M_Stata/tmp.dta') %$%
  data.frame(hhid = HHID, cons = rexpagg/(365*adulteq), weight = hh_wgt)
...
mwi13.coords <- data.frame(hhid = mwi13.geo$HHID, lat = mwi13.geo$lat_modified, lon = mwi13.geo$lon_modified)
# mwi13.hha <- read.dta('data/input/LSMS/MWI_2016_IHS-IV_v02_M_Stata/HH_MOD_A_FILT.dta')
mwi13.rururb <- data.frame(hhid = mwi13.cons$HHID, rururb = mwi13.cons$urban, stringsAsFactors = F)
...
mwi13.vars <- list(mwi13.cons, mwi13.coords, mwi13.room, mwi13.metal) %>%
  Reduce(function(x, y) merge(x, y, by = 'hhid'), .) %>%
  nl(2013)
...
# ## Nigeria ##
# nga13.cons <- read.dta('data/input/LSMS/NGA_2012_GHSP-W2_v02_M_STATA/cons_agg_w2.dta') %$%
#   data.frame(hhid = hhid, cons = pcexp_dr_w2/365)
# nga13.cons$cons <- nga13.cons$cons*110.84/(79.53*100)
# nga13.geo <- read.dta('data/input/LSMS/NGA_2012_GHSP-W2_v02_M_STATA/Geodata Wave 2/NGA_HouseholdGeovars_Y2.dta')
...
# nga13.weight <- read.dta('data/input/LSMS/NGA_2012_GHSP-W2_v02_M_STATA/HHTrack.dta')[,c('hhid', 'wt_wave2')]
# names(nga13.weight)[2] <- 'weight'
# nga13.phhh8 <- read.dta('data/input/LSMS/NGA_2012_GHSP-W2_v02_M_STATA/Post Harvest Wave 2/Household/sect8_harvestw2.dta')
